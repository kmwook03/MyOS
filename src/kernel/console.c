...
void console_task(struct SHEET *sht, int memtotal)
{
    ...
    for (;;) {
			...
			if (256 <= i && i <= 511) {
				if (i == 8 + 256) {
					...
					if (strcmp(cmdline, "mem") == 0) { 
						...
						putfonts8_asc_sht(sht, 8, cursor_y, COL8_FFFFFF, COL8_000000, s, 30);
						cursor_y = cons_newline(cursor_y, sht);
		...
		type_next_file:
						...
						if (x < 224 && finfo[x].name[0] != 0x00) {
							...
							for (y = 0; y < finfo[x].size; y++) {
								...
								if (s[0] == 0x09) {
									...
									putfonts8_asc_sht(sht, cursor_x, cursor_y, COL8_FFFFFF, COL8_000000, s, 1);
									cursor_x += 8;
									if (cursor_x == 8 + 240) {
										cursor_x = 8;
										cursor_y = cons_newline(cursor_y, sht);
						...
						putfonts8_asc_sht(sht, 8, cursor_y, COL8_FFFFFF, COL8_000000, "Bad command.", 12);
						cursor_y = cons_newline(cursor_y, sht);
						cursor_y = cons_newline(cursor_y, sht);
					}
