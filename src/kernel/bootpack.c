...
void HariMain(void)
{
    ...
    io_out8(PIC0_IMR, 0xf9); // 11111001 : allow keyboard(IRQ1) and PIC1
    ...
    sheet_setbuf(sht_mouse, buf_mouse, 16, 16, 99); // transparency color is 99
    ...
    sheet_updown(shtctl, sht_back, 0); // back to bottom
    sheet_updown(shtctl, sht_mouse, 1); // mouse to top
    sprintf(s, "(%d, %d)", mx, my);
        ...
        if (fifo8_status(&keyfifo) + fifo8_status(&mousefifo) == 0) {
            ...
            if (fifo8_status(&keyfifo) != 0) {
                ...
                if (mouse_decode(&mdec, i) != 0) {
                    ...
                    sprintf(s, "(%d, %d)", mx, my);
                    boxfill8(buf_back, binfo->scrnx, COL8_008484, 0, 0, 79, 15); // erase previous coordinate
                    putfonts8_asc(buf_back, binfo->scrnx, 0, 0, COL8_FFFFFF, s); // display new coordinate
                    sheet_refresh(shtctl, sht_back, 0, 0, 80, 16);
                    sheet_slide(shtctl, sht_mouse, mx, my); // move mouse sheet
                }
            }
        }
    }
