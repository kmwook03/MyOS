...
void HariMain(void)
{
    ...
    init_pit();
    io_out8(PIC0_IMR, 0xf8); // 11111000 : allow keyboard(IRQ1) and PIC1
    io_out8(PIC1_IMR, 0xef); // 11101111 : allow mouse(IRQ12)
    
    fifo8_init(&timerfifo, 8, timerbuf);
    timer = timer_alloc();
    timer_init(timer, &timerfifo, 1);
    timer_settime(timer, 1000); // 1000 ticks = 10 seconds

    fifo8_init(&timerfifo2, 8, timerbuf2);
    timer2 = timer_alloc();
    timer_init(timer2, &timerfifo2, 1);
    timer_settime(timer2, 300); // 300 ticks = 3 seconds
    
    fifo8_init(&timerfifo3, 8, timerbuf3);
    timer3 = timer_alloc();
    timer_init(timer3, &timerfifo3, 1);
    timer_settime(timer3, 50); // 50 ticks = 0.5 second
    ...
    sheet_updown(sht_back, 0); // back to bottom
    sheet_updown(sht_win, 1); // windoe to middle
    sheet_updown(sht_mouse, 2); // mouse to top
