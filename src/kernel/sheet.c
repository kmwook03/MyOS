...
struct SHTCTL *shtctl_init(struct MEMMAN *memman, unsigned char *vram, int xsize, int ysize)
{
    ...
    ctl->xsize = xsize;
    ctl->ysize = ysize;
    ctl->top = -1; // no sheets are being used yet
    for (i = 0; i < MAX_SHEETS; i++) {
        ctl->sheets0[i].flags = 0; // mark as unused
        ctl->sheets0[i].ctl = ctl; // back reference
    }
...
void sheet_refreshsub(struct SHTCTL *ctl, int vx0, int vy0, int vx1, int vy1, int h0, int h1)
{
    ...
    for (h=h0; h<=h1; h++) {
        ...
        buf = sht->buf;
        sid = sht - ctl->sheets0; // sheet ID
        //  Use vx0 to vy1 to reverse-calculate bx0 to by1
        bx0 = vx0 - sht->vx0;
        by0 = vy0 - sht->vy0;
        bx1 = vx1 - sht->vx0;
        by1 = vy1 - sht->vy0;
